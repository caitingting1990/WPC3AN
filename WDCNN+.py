...
import numpy as np
x_A=np.load('oral_data/A_data.npy')
x_B=np.load('oral_data/B_data.npy')
y_A=np.load('oral_data/A_label.npy')
y_B=np.load('oral_data/B_label.npy')
x_C=np.load('oral_data/C_data.npy')
x_D=np.load('oral_data/D_data.npy')
y_C=np.load('oral_data/C_label.npy')
y_D=np.load('oral_data/D_label.npy')
...
def block():
    return keras.Sequential([
        keras.layers.Conv1D(16,(64,),8,'same'),
        keras.layers.BatchNormalization(),
        keras.layers.Activation("relu"),
        keras.layers.MaxPooling1D((2,),2),
        keras.layers.Conv1D(32,(3,),1,'same'), #(32,32)
        keras.layers.BatchNormalization(),
        keras.layers.Activation("relu"),
        keras.layers.MaxPooling1D((2,),2),
        keras.layers.Conv1D(64,(3,),1,'same'), #(16,64)
        keras.layers.BatchNormalization(),
        keras.layers.Activation("relu"),
        keras.layers.MaxPooling1D((2,),2),
        keras.layers.Conv1D(64,(3,),1,'same'), #(8,64)
        keras.layers.BatchNormalization(),
        keras.layers.Activation("relu"),
        keras.layers.MaxPooling1D((2,),2),    #(4,64)
        keras.layers.Conv1D(64,(3,),1,'same'), #(4,64)
        keras.layers.BatchNormalization(),
        keras.layers.Activation("relu"),
...
class WDCNN_plus(object):
    ...
    def build_DANN(self):
        ...
        self.grl = GRL()
        src_inputs=keras.Input(shape=(1024,1))
        tar_inputs=keras.Input(shape=(1024,1))
...
for compare_set in compare_sets:
    ...
    x_s=np.load('oral_data/'+compare_set[0]+'_data_1024.npy')
    x_t=np.load('oral_data/'+compare_set[1]+'_data_1024.npy')
    y_s=np.load('oral_data/'+compare_set[0]+'_label_1024.npy')
    y_t=np.load('oral_data/'+compare_set[1]+'_label_1024.npy')
    d_s=np.zeros_like(y_s)
    d_t=np.ones_like(y_t)
    x_s=np.reshape(x_s,newshape=(-1,1024,1))
    x_t=np.reshape(x_t,newshape=(-1,1024,1))
