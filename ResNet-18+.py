...
def block0():#(32,32,1)#(8,8,32)
    return keras.Sequential([keras.layers.Conv2D(32,(7,7),2,'same'),
                             keras.layers.MaxPooling2D((2,2),2,'same')
    ])

class block1(keras.layers.Layer):
    def __init__(self):
        super(block1, self).__init__()
        self.cv1=keras.layers.Conv2D(32,(3,3),1,'same')
        self.bn1=keras.layers.BatchNormalization()
        self.cv2=keras.layers.Conv2D(32,(3,3),1,'same')
        self.bn2=keras.layers.BatchNormalization()
        self.cv3=keras.layers.Conv2D(32,(3,3),1,'same')
        self.bn3=keras.layers.BatchNormalization()
        self.cv4=keras.layers.Conv2D(32,(3,3),1,'same')
...
class block2(keras.layers.Layer):
    def __init__(self):
        ...
        self.cv1=keras.layers.Conv2D(64,(3,3),2,'same')
        self.bn1=keras.layers.BatchNormalization()
        self.cv2=keras.layers.Conv2D(64,(3,3),1,'same')
        self.bn2=keras.layers.BatchNormalization()
        self.cv2_plus=keras.layers.Conv2D(64,(1,1),2,'same')
        self.cv3=keras.layers.Conv2D(64,(3,3),1,'same')
        self.bn3=keras.layers.BatchNormalization()
        self.cv4=keras.layers.Conv2D(64,(3,3),1,'same')
        self.bn4=keras.layers.BatchNormalization()
    def call(self, inputs,training=None):
        tmp=self.cv1(inputs) #(4,4,64)
        tmp=self.bn1(tmp)
        tmp=tf.nn.relu(tmp)
        tmp=self.cv2(tmp)
        tmp=self.bn2(tmp)
        tmp0=self.cv2_plus(inputs)
...
class block3(keras.layers.Layer):
     def __init__(self):
        ...
        self.cv1=keras.layers.Conv2D(128,(3,3),2,'same')
        self.bn1=keras.layers.BatchNormalization()
        self.cv2=keras.layers.Conv2D(128,(3,3),1,'same')
        self.bn2=keras.layers.BatchNormalization()
        self.cv2_plus=keras.layers.Conv2D(128,(1,1),2,'same')
        self.cv3=keras.layers.Conv2D(128,(3,3),1,'same')
        self.bn3=keras.layers.BatchNormalization()
        self.cv4=keras.layers.Conv2D(128,(3,3),1,'same')
        self.bn4=keras.layers.BatchNormalization()
     def call(self,inputs,training=None):
        tmp=self.cv1(inputs) #(2,2,128)
        tmp=self.bn1(tmp)
        tmp=tf.nn.relu(tmp)
        tmp=self.cv2(tmp)
        tmp=self.bn2(tmp)
        tmp0=self.cv2_plus(inputs)
        tmp1=tf.nn.relu(tmp0)+tmp  #(2,2,128)
        tmp=self.cv3(tmp1)
        tmp=self.bn3(tmp)
        tmp=tf.nn.relu(tmp)
        tmp=self.cv4(tmp)
...
class block4(keras.layers.Layer):
    def __init__(self):
        super(block4, self).__init__()
        self.cv1=keras.layers.Conv2D(256,(3,3),1,'same')
        self.bn1=keras.layers.BatchNormalization()
        self.cv2=keras.layers.Conv2D(256,(3,3),1,'same')
        self.bn2=keras.layers.BatchNormalization()
        self.cv2_plus=keras.layers.Conv2D(256,(1,1),1,'same')
        self.cv3=keras.layers.Conv2D(256,(3,3),1,'same')
        self.bn3=keras.layers.BatchNormalization()
        self.cv4=keras.layers.Conv2D(256,(3,3),1,'same')
        self.bn4=keras.layers.BatchNormalization()
    def call(self, inputs,training=None):
        tmp=self.cv1(inputs) #(2,2,128)
        tmp=self.bn1(tmp)
